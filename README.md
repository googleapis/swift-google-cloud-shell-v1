...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-shell-v1.git --from 0.3.0
```

Then add `GoogleCloudShellV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudShellV1 <target-name> --package swift-google-cloud-shell-v1
```
